...
struct Expr
{
    ...
    virtual void eval(EvalState & state, Env & env, ValueIdx & v);
    virtual ValueIdx maybeThunk(EvalState & state, Env & env);
...
#define COMMON_METHODS \
    ...
    void eval(EvalState & state, Env & env, ValueIdx & v) override; \
...
struct ExprInt : Expr
{
    NixInt n;
    ValueIdx v;
    ExprInt(NixInt n, EvalState& state ) : n(n) { v = state.values.mkInt(n); };
    ValueIdx maybeThunk(EvalState & state, Env & env) override;
    COMMON_METHODS
};
...
struct ExprFloat : Expr
{
    NixFloat nf;
    ValueIdx v;
    ExprFloat(NixFloat nf, EvalState& state) : nf(nf) { v = state.values.mkFloat(nf); };
    ValueIdx maybeThunk(EvalState & state, Env & env) override;
    COMMON_METHODS
};
...
struct ExprString : Expr
{
    std::string s;
    ValueIdx v;
    ExprString(std::string &&s, EvalState& state) : s(std::move(s)) {  v = state.values.mkString(this->s.data()); };
    ValueIdx maybeThunk(EvalState & state, Env & env) override;
    COMMON_METHODS
};
...
struct ExprPath : Expr
{
    std::string s;
    ValueIdx v;
    ExprPath(std::string s, EvalState& state) : s(std::move(s)) {  v = state.values.mkPath(this->s.c_str()); };
    ValueIdx maybeThunk(EvalState & state, Env & env) override;
    COMMON_METHODS
};
